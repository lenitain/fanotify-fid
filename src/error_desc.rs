...
pub fn errno_desc_init(code: i32) -> Cow<'static, str> {
    match code {
        libc::EINVAL => Cow::Borrowed(
            "Invalid flags — check FAN_REPORT_NAME requires FAN_REPORT_DIR_FID",
        ),
        libc::EMFILE => Cow::Borrowed(
            "Too many fanotify groups (per-user limit: 128)",
        ),
        libc::ENOMEM => Cow::Borrowed("Out of memory"),
        libc::ENOSYS => Cow::Borrowed(
            "Kernel does not support fanotify (CONFIG_FANOTIFY missing)",
        ),
...
pub fn errno_desc_mark(code: i32) -> Cow<'static, str> {
    match code {
        ...
        libc::ENODEV => Cow::Borrowed(
            "Filesystem does not support fsid",
        ),
        libc::ENOENT => Cow::Borrowed("Path does not exist"),
        libc::ENOMEM => Cow::Borrowed("Out of memory"),
        libc::ENOSPC => Cow::Borrowed(
            "Too many marks (exceeded 8192 limit)",
        ),
        libc::ENOSYS => Cow::Borrowed(
            "Kernel does not implement fanotify_mark",
        ),
        libc::ENOTDIR => Cow::Borrowed(
...
pub fn errno_desc_handle(code: i32) -> Cow<'static, str> {
    match code {
        ...
        libc::ENOENT => Cow::Borrowed(
            "File or directory does not exist (deleted or handle stale)",
        ),
